...
import numpy
import yaml
from dynagslam.utils.general_utils import devB, devF, devI, quaternion_from_axis_angle
...
def sample_pixels(
    ...
    select_mask_true_idx = torch.nonzero(select_mask)
    if static:
        select_mask_true_sample_idx = torch.randperm(select_mask_true_idx.shape[0])[:uniform_sample_num]
    else:  
        select_mask_true_sample_idx = torch.arange(select_mask_true_idx.shape[0])[:uniform_sample_num]
    select_mask_true_idx = select_mask_true_idx[select_mask_true_sample_idx]
    sample_mask = torch.zeros((select_mask.shape[0])).bool().cuda()
    sample_mask[select_mask_true_idx] = True
    points_uniform = vertex_map.view(-1, 3)[sample_mask]
    colors_uniform = color_map.view(-1, 3)[sample_mask]
    normals_uniform = normal_map.view(-1, 3)[sample_mask]
    sample_mask = sample_mask.view(H, W)
    return points_uniform.view(uniform_sample_num, 3), normals_uniform.view(uniform_sample_num, 3), colors_uniform.view(uniform_sample_num, 3), sample_mask, select_mask_true_sample_idx
...
def build_normal_pyramid(vertex_pyramid):
    ...
    for i in range(len(vertex_pyramid)):
        normal_pyramid.append(compute_normal_map(vertex_pyramid[i]))
    return normal_pyramid
...
def tensor_to_point_cloud(tensor):
    """Convert PyTorch tensor to Open3D PointCloud."""
...
def register_point_clouds(pcd_A, pcd_B):
    """Register two point clouds using ICP."""
    
    reg_icp = o3d.pipelines.registration.registration_icp(
        pcd_A, pcd_B, max_correspondence_distance=0.05,
        estimation_method=o3d.pipelines.registration.TransformationEstimationPointToPoint(),
        criteria=o3d.pipelines.registration.ICPConvergenceCriteria(max_iteration=2000)
    )
    
    pcd_B.transform(reg_icp.transformation)
    return pcd_A, pcd_B

def compute_point_cloud_difference(pcd_A, pcd_B, threshold=2e-4):
    """Compute differences between two point clouds."""
    points_A = np.asarray(pcd_A.points)
    points_B = np.asarray(pcd_B.points)
    
    kdtree_A = o3d.geometry.KDTreeFlann(pcd_A)
    
    moving_points = []
    for point in points_B:
        [_, idx, dist] = kdtree_A.search_knn_vector_3d(point, 1)
        dist = np.sqrt(dist[0])*2
        if dist > threshold:
            moving_points.append(point)
    
    return np.array(moving_points)

def visualize_point_clouds(pcds, colors):
    """Visualize multiple point clouds using Open3D with specified colors."""
    '''
    if len(pcds) != len(colors):
        raise ValueError("Number of point clouds must match number of colors.")'''
    
    # Set colors for each point cloud
