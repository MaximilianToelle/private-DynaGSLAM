...
from SLAM.render_dyna import Renderer as Renderer_Dyna
...
from .estimate_flow import estimate_flow
'''
from .motion_segmentation import Motion_Segmentation
...
class Mapping(object):
    def __init__(self, args, recorder=None) -> None:
        ...
        self.renderer_dyna = Renderer_Dyna(args)
        self.optimizer = None
        self.time = 0
        self.iter = 0
        ...
        self.motion_masks = []
        self.motion_masks_cum = []
        self.stable_mask = None #the mask index of stable GS from all GS
        ...
        '''
        #utils for motion segmentation model
        self.transform = ResizeLongestSide(1024)
        self.point_grids = build_all_layer_point_grids(n_per_side = 20, n_layers = 0, scale_per_layer = 1)[0]
        # Setup grid points'''
        
    def mapping(self, frame, frame_map, frame_id, optimization_params, semantic, flow_gt):
        
        #motion_seg_sum = None
        #if frame_id == 0:
        #self.motion_masks.append(semantic) #None
        self.frame_map = frame_map
        #if frame_id == 0:
        self.gaussians_add(frame, frame_id)
        self.processed_frames.append(frame)
        self.processed_map.append(frame_map)

        if frame_id>0:
            # the est flow is pointing from t+1 to t!!!!
            with torch.no_grad():
                flow_gt = estimate_flow(self.processed_map[-2]['color_map'], self.processed_map[-1]['color_map'], frame_id)
        else:
            flow_gt = None
        ...
        self.gaussians_fix(movinggs_mask)
        #self.gaussians_delete() # comment out only for debug
        move_to_cpu(frame)
        self.old_xyz = self.pointcloud._xyz

    def gaussians_add(self, frame, frame_id):
    ...
    def local_optimize(self, frame, frame_id, motion_mask_curr, flow_gt, update_args):
        ...
        for frame in self.processed_frames:
            render_mask, tile_mask, render_ratio = self.evaluate_render_range(frame) #render_mask is True as long as it's unstable - sum opacity!=1
            ...
            if self.verbose:
                ...
                if tile_mask is not None:
                    tile_ratio = tile_mask.sum() / torch.numel(tile_mask)
                print("tile mask ratio: {:f}".format(tile_ratio))
        print(
        ...
        with tqdm(total=gaussian_update_iter, desc="map update") as pbar:
            with torch.no_grad(): 
                opt_frame_last = self.processed_frames[-1]
                opt_tile_mask_last = tile_masks[-1]
                with torch.no_grad():
                    render_output_last = self.renderer.render(
                    opt_frame_last,
                    self.global_params,
                    tile_mask=opt_tile_mask_last,
                    )
                
                    movinggs_idx_last = render_output_last['depth_index_map'].squeeze(0)[torch.from_numpy(motion_mask_curr).bool()]
                    movinggs_idx = torch.unique(movinggs_idx_last)
                    if movinggs_idx[0] == -1:
                        movinggs_idx = movinggs_idx[1:]
                    if frame_id > 0 and frame_id < 10:
                        self.movinggs_idx_total = self.movinggs_idx_total # only use the segmentation from 1st frame 
                    elif frame_id == 0:
                        self.movinggs_idx_total = movinggs_idx
                    else: 
                        self.movinggs_idx_total = self.movinggs_idx_total # only use the segmentation from 1st frame 
                    self.movinggs_idx_total = torch.unique(self.movinggs_idx_total)
                    self.movinggs_idx_old = self.movinggs_idx_total
                
                    movinggs_mask = torch.zeros(self.global_params['xyz'].shape[0]).bool().to(movinggs_idx.device)
                    movinggs_mask[self.movinggs_idx_total.long()] = True
                
                # 12/31 task, now the movinggs_mask and movinggs_idx are the movinggs index of all GS, you have to find the movinggs index of static GS
                '''
                staticgs_mask = torch.ones(self.global_params['xyz'].shape[0]).bool().to(movinggs_idx.device)
                staticgs_mask[self.movinggs_idx_total.long()] = False

                # visualize moving and static gs
                if self.stable_mask is not None:
                    tensor_A = self.pointcloud._xyz[:,0,:][movinggs_mask[~self.stable_mask]].detach()
                    tensor_B = self.pointcloud._xyz[:,0,:][staticgs_mask[~self.stable_mask]].detach()
                    if self.get_stable_num > 0:
                        tensor_C = self.stable_pointcloud._xyz[:,0,:].detach()
                    else:
                        tensor_C = self.stable_pointcloud._xyz.detach()
                else:
                    tensor_A = self.pointcloud._xyz[:,0,:][movinggs_mask].detach()
                    tensor_B = self.pointcloud._xyz[:,0,:][staticgs_mask].detach()
                    if self.get_stable_num > 0:
                        tensor_C = self.stable_pointcloud[:,0,:]._xyz.detach()
                    else:
                        tensor_C = self.stable_pointcloud._xyz.detach()
                pcd_A = tensor_to_point_cloud(tensor_A)
                pcd_B = tensor_to_point_cloud(tensor_B)
                pcd_C = tensor_to_point_cloud(tensor_C)
                color_A = [1, 0, 0]  # Red color for PointCloud A
                color_B = [0, 1, 0]  # Green color for PointCloud B
                color_C = [0, 0, 1]  # Green color for PointCloud c
                visualize_point_clouds([pcd_A, pcd_B, pcd_C], [color_A, color_B, color_C])'''
            ...
            for iter in range(gaussian_update_iter):
                ...
                # compute loss
                render_output = self.renderer_dyna.render(
                    opt_frame,
                    self.global_params,
                    movinggs_mask, #movinggs_idx.long(),
                    self.time,
                    iter,
        ...
        self.pointcloud.detach() # shouldn't detach xyz and rotation because it's being optimized over time(frame)
        self.iter = 0
        self.history_merge(history_stat, movinggs_mask, self.history_merge_max_weight)
        return movinggs_mask

    def history_merge(self, history_stat, movinggs_mask, max_weight=0.5):
        if max_weight <= 0:
            return
        history_weight = (
            max_weight
        ...
        if self.stable_mask is not None:
            movinggs_mask = movinggs_mask[~self.stable_mask]

        history_weight_dynamic = history_weight.clone()
        history_weight_dynamic[movinggs_mask] = 0.  # shouldn't use history for xyz and rotation
        
        xyz_merge = (
            history_stat["xyz"] * history_weight_dynamic[:,None,:].repeat(1,5,1)
            + (1 - history_weight_dynamic[:,None,:].repeat(1,5,1)) * self.pointcloud._xyz
        )
    ...
    def gaussians_fix(self, movinggs_mask, mask=None):
        if mask is None:
            
            confidence_mask = (
                self.pointcloud.get_confidence.detach().clone() > self.stable_confidence_thres
            ).squeeze()
            if self.stable_mask is not None:
                movinggs_mask = movinggs_mask[~self.stable_mask]
            confidence_mask[movinggs_mask] = False
        ...
        if stable_mask.sum() > 0:
            #stable_mask = torch.zeros_like(stable_mask).bool()
        ...
        # stable_mask here is the new stable map over old stable GS, but we need to make the self.stable_mask to be the new stable map over all GS, not just the old stable GS
        if self.stable_mask is not None:
            with torch.no_grad():
                self.stable_mask[torch.nonzero(~self.stable_mask)] = stable_mask.unsqueeze(1)
        else: 
            with torch.no_grad():
    ...
    def loss_update(
        ...
        # optical flow loss
        flow_loss = devF(torch.tensor(0)) #0
        if render_output['flow'] is not None:
            flow_gt = flow_gt.squeeze(0).permute(1,2,0)
            flow_est = render_output['flow'].permute(1,2,0)[motion_mask]
            flow_gt = flow_gt[motion_mask]
            flow_loss = torch.sum(torch.abs(flow_est-flow_gt))/flow_est.shape[0]
        
        '''
        if self.frame_id != 0 and iter == 0:
            ...
            plt.imshow(render_mask.detach().cpu().numpy(), cmap=cmap, norm=norm)
            plt.colorbar(ticks=[-5., 0., 1., 2., 3., 4.], label='Semantic Labels')
            plt.title('render_mask')
            plt.show()'''
        
        if depth is not None and update_args.depth_weight > 0:
            depth_error = depth - image_input["depth_map"]
            '''
            dm1 = depth.squeeze(-1).detach().cpu().numpy()
            dm2 = image_input["depth_map"].squeeze(-1).detach().cpu().numpy()
            fig, axes = plt.subplots(1, 2, figsize=(12, 6))
            axes[0].imshow(dm1, cmap='magma')
            axes[0].set_title('Depth Map 1')
            axes[0].axis('off')  # Hide axis ticks/labels if desired
            axes[1].imshow(dm2, cmap='magma')
            axes[1].set_title('Depth Map 2')
            axes[1].axis('off')  # Hide axis ticks/labels if desired
            plt.tight_layout()
            plt.show()
            '''
            ...
            valid_depth_mask = valid_depth_mask | torch.from_numpy(motion_mask).to(valid_depth_mask.device)
            depth_loss = torch.abs(depth_error[valid_depth_mask]).mean()
            '''
            # visualize depth mask
            if self.frame_id != 0 and iter==0:
                plt.figure(figsize=(8, 6))
                plt.imshow(valid_depth_mask.detach().cpu().numpy(), cmap=cmap, norm=norm)
                plt.colorbar(ticks=[-5., 0., 1., 2., 3., 4.], label='Semantic Labels')
                plt.title('render_mask')
                plt.show()'''
    ...
    def global_optimization(
        ...
        if select_keyframe_num == -1:
            self.gaussians_fix(mask=(self.pointcloud.get_confidence > -1)) #convert all unstable to stable
        print(
        ...
        # fix xyz don't update xyz but update other GS properties
        if select_keyframe_num != -1:
            l[0]["lr"] = 0
    ...
    # Remove temp points that fall within a distance to the existing unstable Gaussian, keep those faraway temp pts to add later
    def temp_points_filter(self, topk=3):
        if self.get_unstable_num > 0:
            ...
            if torch.numel(exist_xyz) > 0:
                inbbox_mask = bbox_filter(temp_xyz[:, 0, :], exist_xyz[:, 0, :]) #find the exist_xyz that is in between min/max temp
            ...
            corr_radius = exist_raidus[nn_indices] * 0.6 #the higher the corr_radius set (increase 0.6), the more close new GS to the old GS can be removed
    ...
    def temp_points_attach(self, frame: Camera, unstable_opacity_low=0.1):
        if self.get_stable_num == 0:
            return
        # filter low opacity unstable gaussians, project unstable gaussians and compute uv
    ...
    def temp_to_optimize(self):
        ...
        print("self.temp_pointcloud._xyz_fuck", self.temp_pointcloud._xyz.shape)
        num_new_points = self.temp_pointcloud._xyz.shape[0]
        if self.verbose:
            print("===== points add =====")
            print(
        ...
        # extend self.stable_mask with the new added GS. Just add N number of False at the end of self.stable_mask since the global point is to add N number of new added points at its end, and the new added points are all first regarded as unstable points
        if self.stable_mask is not None:
            self.stable_mask = torch.cat([self.stable_mask, torch.zeros(num_new_points).bool().to(self.stable_mask.device)])
    
        '''
        # visualize existing and extra gs
        if len(self.pointcloud._xyz.shape) == 3:
            tensor_A = self.pointcloud._xyz[:,0,:].detach()
            tensor_B = temp_params['xyz'][:,0,:].detach()
            pcd_A = tensor_to_point_cloud(tensor_A)
            pcd_B = tensor_to_point_cloud(tensor_B)
            color_A = [1, 0, 0]  # Red color for PointCloud A
            color_B = [0, 1, 0]  # Green color for PointCloud B
            visualize_point_clouds([pcd_A, pcd_B], [color_A, color_B])'''
    ...
    def stable_params(self):
        ...
        confidence = (
            self.stable_pointcloud.get_confidence if have_stable else torch.empty(0).detach().requires_grad_(False)
        )
        stable_params = {
            "xyz": devF(xyz),
            ...
            "radius": devF(radius),
            "normal": devF(normal),
            "confidence": devF(confidence).detach().requires_grad_(False),
        }
    ...
    def unstable_params(self):
        ...
        confidence = self.pointcloud.get_confidence if have_unstable else torch.empty(0).detach().requires_grad_(False)
        unstable_params = {
            "xyz": devF(xyz),
            "opacity": devF(opacity),
            "scales": devF(scales),
    ...
    def global_params(self):
        ...
        global_prams = {
            ...
            "radius": radius,
            "normal": normal,
            "confidence": confidence.detach().requires_grad_(False),
        }
        return global_prams
