...
class ICP(nn.Module):
    ...
    def icp(self, pose10,vertex_t0,vertex_t1,normal_t0,normal_t1, K, semantic_old, semantic, depth_highgrad_t0, depth_highgrad_t1, img0, img1):
        mask0 = (vertex_t0[..., -1] > 0.0)
        mask0 = mask0 & (~semantic_old) & (~depth_highgrad_t0)
        for idx in range(self.max_iterations):
            # compute residuals
            res_geom, J_F_p_geom, valid_mask = self.compute_residuals_jacobian(
            ...
            res_photo, J_F_p_photo = self.compute_photo_residual_jacobian(
                img0, img1, vertex_t0, pose10, K, valid_mask
            )
            # weight photometric contribution
            res_photo = 0.5*res_photo
            J_F_p_photo = 0.5*J_F_p_photo
            
            # Stack the two costs
            res = torch.cat([res_geom, res_photo], dim=0)  # [N_geom+N_photo, 1]
            J_F_p = torch.cat([J_F_p_geom, J_F_p_photo], dim=0)  # [N_geom+N_photo, 1, 6]

            JtWJ = self.compute_jtj(J_F_p)  # [B, 6, 6]
            JtR = self.compute_jtr(J_F_p, res)
            pose10 = self.GN_solver(JtWJ, JtR, pose10, damping=self.damping)
        #plt.imshow(valid_mask.detach().cpu().numpy(), cmap="gray")
        #plt.show()
        H,W = vertex_t0.shape[:2]
        valid_ratio = valid_mask.sum() / H / W
        return pose10, valid_ratio, valid_mask
    ...
    @staticmethod
    def compute_photo_residual_jacobian(img0, img1, vertex0, pose10, K, valid_mask):
        """
        Computes photometric residual and Jacobian for an RGB image.
          - img0: reference RGB image at t0 (HxWx3, values typically in [0,1])
          - img1: target RGB image at t1 (HxWx3)
          - vertex0: the depth-to-vertex map at t0 (HxWx3)
          - pose10: current SE(3) pose estimate from t0 to t1
          - K: intrinsic matrix, same as before
          - valid_mask: the valid mask from geometric correspondence (to restrict photometric errors)

        We compute the photometric error for each of the 3 channels and then average
        them to obtain a single residual per pixel. The Jacobians are computed accordingly.
        """
        # Transform vertex0 using the current pose
        R = pose10[:3, :3]
        t = pose10[:3, 3]
        H, W, _ = vertex0.shape
        vertex0_trans = (R @ vertex0.view(-1, 3).permute(1, 0)).permute(1, 0).view(H, W, 3) + t[None, None, :]

        # Project the transformed 3D points onto the target image plane
        fx, fy, cx, cy = K[0, 0], K[1, 1], K[0, 2], K[1, 2]
        x, y, z = vertex0_trans[..., 0], vertex0_trans[..., 1], vertex0_trans[..., 2]
        u = (x / z) * fx + cx
        v = (y / z) * fy + cy

        # Warp the reference image (img0) to the target view using grid_sample.
        # Here, img0 is HxWx3.
        warped_img0 = warp_features(img0, u, v, mode="bilinear")  # result shape HxWx3

        # Compute photometric residual: difference between the warped img0 and img1.
        # We then average over the color channels.
        res_photo_all = (img1 - warped_img0)  # [H, W, 3]
        # Only consider valid pixels (same valid_mask applies to all channels)
        res_photo_all = res_photo_all[valid_mask]  # shape [N_valid, 3]
        # Average residual over the 3 channels (alternatively, you could stack all channels)
        res_photo = res_photo_all.mean(dim=1, keepdim=True)  # [N_valid, 1]

        # Compute the Jacobian for photometric error.
        # 1. Compute image gradients for each channel on the reference image. 
        #    We compute gradients on the original image and then sample at (u,v).
        grad_x, grad_y = image_gradient_rgb(img0)
        grad_x_warp = warp_features(grad_x, u, v, mode="bilinear").squeeze(-1)  # still [H,W,3]
        grad_y_warp = warp_features(grad_y, u, v, mode="bilinear").squeeze(-1)  # [H,W,3]

        # Average the gradients over channels
        grad_x_mean = grad_x_warp.mean(dim=-1)  # [H,W]
        grad_y_mean = grad_y_warp.mean(dim=-1)  # [H,W]

        # Combine into a 2-channel gradient [H,W,2]
        grad_I = torch.stack([grad_x_mean, grad_y_mean], dim=-1)
        grad_I = grad_I[valid_mask]  # [N_valid, 2]

        # 2. Derivative of the projection (Jacobian from 3D point to image coordinate)
        # For a 3D point (x, y, z), the projection is:
        #  u = (fx*x/z) + cx,  v = (fy*y/z) + cy.
        # Hence, the derivative d[u,v]/d[x,y,z] is:
        #  [ fx/z,      0,  -fx*x/z^2 ]
        #  [    0,  fy/z,  -fy*y/z^2 ]
        #
        # For each valid point, extract x,y,z.
        x_valid = x[valid_mask].view(-1, 1)
        y_valid = y[valid_mask].view(-1, 1)
        z_valid = z[valid_mask].view(-1, 1)

        # We compute the 2x3 Jacobian for each point:
        dproj_list = []
        N_valid = x_valid.shape[0]
        for i in range(N_valid):
            dproj_i = torch.tensor([
                [fx / z_valid[i, 0], 0, -fx * x_valid[i, 0] / (z_valid[i, 0]**2)],
                [0, fy / z_valid[i, 0], -fy * y_valid[i, 0] / (z_valid[i, 0]**2)]
            ], device=x_valid.device, dtype=x_valid.dtype)
            dproj_list.append(dproj_i)
        dproj = torch.stack(dproj_list, dim=0)  # [N_valid, 2, 3]

        # 3. Compute the Jacobian of the 3D point with respect to the pose parameters.
        # For a small update in pose, the Jacobian is:
        #   d[v] / d[xi] = [ -[v]_x,  I ]
        # where v is the 3D point (vertex0_trans for the valid pixel).
        v_valid = vertex0_trans[valid_mask].view(-1, 3)  # [N_valid, 3]
        J_pose_list = []
        for i in range(N_valid):
            skew_v = batch_skew(v_valid[i:i+1])  # [1, 3, 3]
            J_rot_i = -skew_v.squeeze(0)  # [3, 3]
            J_trans_i = torch.eye(3, device=v_valid.device, dtype=v_valid.dtype)
            J_i = torch.cat([J_rot_i, J_trans_i], dim=1)  # [3, 6]
            J_pose_list.append(J_i)
        J_pose = torch.stack(J_pose_list, dim=0)  # [N_valid, 3, 6]

        # 4. Chain the derivatives: dI/dxi = grad_I (1x2) * dproj (2x3) * J_pose (3x6)
        J_photo_list = []
        for i in range(N_valid):
            # grad_I[i] is [2], dproj[i] is [2,3], J_pose[i] is [3,6]
            dI_dv = grad_I[i].unsqueeze(0) @ dproj[i]  # shape [1,3]
            J_photo_i = dI_dv @ J_pose[i]  # shape [1,6]
            J_photo_list.append(J_photo_i)
        J_photo = torch.cat(J_photo_list, dim=0)  # [N_valid, 6]
        # Reshape to [N_valid, 1, 6]
        J_photo = J_photo.view(-1, 1, 6)

        return res_photo, J_photo
...
def image_gradient_rgb(img):
    """
    Computes image gradients for an RGB image.
    Input:
       img: an RGB image of shape [H, W, 3]
    Output:
       grad_x, grad_y: both of shape [H, W, 3], computed for each channel.
    """
    # Prepare the Sobel kernels for x and y gradients.
    sobel_x = torch.tensor([[-1., 0., 1.],
                            [-2., 0., 2.],
                            [-1., 0., 1.]], device=img.device).repeat(3,1,1,1) #view(1, 1, 3, 3)
    sobel_y = torch.tensor([[-1., -2., -1.],
                            [ 0.,  0.,  0.],
                            [ 1.,  2.,  1.]], device=img.device).repeat(3,1,1,1) #.view(1, 1, 3, 3)
    
    # The input image has shape [H, W, 3]; we need to process per channel.
    # Permute to [1, 3, H, W]
    img_perm = img.permute(2, 0, 1).unsqueeze(0)  # [1, 3, H, W]
    # Pad the image for valid convolution.
    img_pad = F.pad(img_perm, (1, 1, 1, 1), mode='replicate')
    grad_x = F.conv2d(img_pad, sobel_x, padding=0, groups=3)  # [1,3,H,W]
    grad_y = F.conv2d(img_pad, sobel_y, padding=0, groups=3)  # [1,3,H,W]
    # Permute back to [H, W, 3]
    grad_x = grad_x.squeeze(0).permute(1, 2, 0)
    grad_y = grad_y.squeeze(0).permute(1, 2, 0)
...
class IcpTracker:
    ...
    def predict_pose(self, frame, semantic, semantic_old, rgb_old):
        K = frame["K"]
        ...
        if self.vertex_pyramid_t0 is None:
            ...
            for level in range(levels):
                ...
                rgb_t0_down = F.interpolate(rgb_old.permute(2,0,1).unsqueeze(0), scale_factor=downscale, mode='bilinear', align_corners=False).squeeze(0).permute(1,2,0)
                rgb_t1_down = F.interpolate(frame['color_map'].permute(2,0,1).unsqueeze(0), scale_factor=downscale, mode='bilinear', align_corners=False).squeeze(0).permute(1,2,0)
                ...
                pose_t1_t0, valid_ratio, valid_mask = self.icp_trackers[level].icp(
                    ...
                    K_downscale, semantic_old_down, semantic_down, depth_highgrad_t0_down, depth_highgrad_t1_down, rgb_t0_down, rgb_t1_down 
                )
            
            
            pose_t1_t0 = pose_t1_t0.detach().cpu().numpy()
