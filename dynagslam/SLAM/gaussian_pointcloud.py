...
from dynagslam.utils.sh_utils import RGB2SH, SH2RGB
...
class GaussianPointCloud(object):
    def setup_functions(self):
        ...
        ##very important cuz this function by default normalize dim=1 which can't be used for size like [N,2,4]
    ...
    def copy(self, ref_gs):
        ...
        self._rotation = ref_gs._rotation.detach()
        self._opacity = ref_gs._opacity.detach()
    ...
    def parametrize_dyna(self, update_args):
        #self._xyz = nn.Parameter(self._xyz.requires_grad_(True))
        self._features_dc = nn.Parameter(self._features_dc.requires_grad_(True))
        self._features_rest = nn.Parameter(self._features_rest.requires_grad_(True))
        self._scaling = nn.Parameter(self._scaling.requires_grad_(True))
        ...
        self._opacity = nn.Parameter(self._opacity.requires_grad_(True))
        l = [
            #{"params": [self._xyz],"lr": update_args.position_lr, "name": "xyz",},
            {
                "params": [self._features_dc],
                "lr": update_args.dyna_feature_lr,
                "name": "f_dc",
            },
            {
                "params": [self._features_rest],
                "lr": update_args.dyna_feature_lr / 20.0,
                "name": "f_rest",
            },
            {
                "params": [self._opacity],
                "lr": update_args.dyna_opacity_lr,
                "name": "opacity",
            },
            {
                "params": [self._scaling],
                "lr": update_args.dyna_scaling_lr,
                "name": "scaling",
            },
            {
                "params": [self._rotation],
                "lr": update_args.dyna_rotation_lr,
                "name": "rotation",
            },
        ]
        return l
    ...
    def cat(self, parameters):
        if len(parameters['xyz'].shape) != 3:
            # now only cat 4 more dim for a level2 fourier series
            temp = torch.cat((parameters["xyz"].unsqueeze(1), torch.zeros((parameters["xyz"].shape[0], 4, parameters["xyz"].shape[1])).to(parameters["xyz"].device)), dim = 1)
        ...
        self._scaling = torch.cat([self._scaling, parameters["scaling"]])
        if len(parameters['rotation'].shape) != 3:
            # now only cat 1 more dim for a level2 linear estimator
            temp = torch.cat((parameters["rotation"].unsqueeze(1), torch.zeros((parameters["rotation"].shape[0], 1, parameters["rotation"].shape[1])).to(parameters["rotation"].device)), dim = 1)
            self._rotation = torch.cat([self._rotation, temp], dim=0) #(50000, 2, 4)
        else:
            self._rotation = torch.cat([self._rotation, parameters["rotation"]], dim=0)
        self._opacity = torch.cat([self._opacity, parameters["opacity"]])
        self._confidence = torch.cat([self._confidence, parameters["confidence"]])
        self._normal = self.get_normal
        self._add_tick = torch.cat([self._add_tick, parameters["add_tick"]])
        self._depth_error_counter = torch.cat(
            [self._depth_error_counter, parameters["depth_error_counter"]]
        )
    ...
    def update_geometry(self, extra_xyz, extra_radius): # only add those new xyz that far enough from old xyz, delete too close xyz
        xyz = self.get_xyz
        radius = self.get_radius
        points_num = self.get_points_num
        if torch.numel(extra_xyz) > 0 and torch.numel(xyz) > 0:
            inbbox_mask = bbox_filter(xyz[:,0,:], extra_xyz[:,0,:])
        ...
        knn_dist, knn_indices = distCUDA2(total_xyz[:,0,:].float().cuda())
        knn_indices = knn_indices[:points_num].long()
        
        dist_0 = (
            torch.norm(xyz[:, 0, :] - total_xyz[:, 0, :][knn_indices[:, 0]], p=2, dim=1)
            - 3 * total_radius[knn_indices[:, 0]]
        )
        dist_1 = (
            torch.norm(xyz[:, 0, :] - total_xyz[:, 0, :][knn_indices[:, 1]], p=2, dim=1)
            - 3 * total_radius[knn_indices[:, 1]]
        )
        dist_2 = (
            torch.norm(xyz[:, 0, :] - total_xyz[:, 0, :][knn_indices[:, 2]], p=2, dim=1)
    ...
    def construct_list_of_attributes(self, include_confidence=True):
        #l = ["x", "y", "z", "nx", "ny", "nz", "rot_0", "rot_1", "rot_2", "rot_3"]
        l = []
    ...
    def save_model_ply(self, path, include_confidence=True):
        ...
        normals = np.zeros((xyz.shape[0], 3)) #np.zeros_like(xyz)
        f_dc = (
