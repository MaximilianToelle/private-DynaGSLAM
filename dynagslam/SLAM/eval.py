...
from dynagslam.utils.loss_utils import l1_loss, ssim, psnr
...
def eval_picture(
    ...
    # check others
    gt_image_dyna = (gt_image.view(3,-1)[:, dyna_mask.flatten()]).view(3,-1)
    image_dyna = (image.view(3,-1)[:, dyna_mask.flatten()]).view(3,-1)
    psnr_value_dyna = psnr(gt_image_dyna, image_dyna).mean()
    ...
    if save_picture:
        ...
        torchvision.utils.save_image(
            image,
            os.path.join(save_path, "color.jpg"),
        )
        torchvision.utils.save_image(
            gt_image,
            os.path.join(save_path, "color_gt.jpg"),
        )
        torchvision.utils.save_image(
            image_error,
            os.path.join(save_path, "color_error.jpg"),
        )
    ...
    # save log
    if torch.sum(torch.tensor(dyna_mask, dtype=torch.int64)) == 0:
        psnr_value_dyna = torch.tensor(0)
    log_info = "valid pixel ratio={:.2%}, color loss={:.3f}, depth loss={:.3f}cm, normal loss={:.3f}, psnr={:.3f}, psnr_dyna={:.3f}".format(
        valid_pixel_ratio, color_loss, depth_loss * 100, normal_loss, psnr_value, psnr_value_dyna
    )
    print(log_info)
    losses = {
        ...
        "psnr_dyna": psnr_value_dyna.item(),
