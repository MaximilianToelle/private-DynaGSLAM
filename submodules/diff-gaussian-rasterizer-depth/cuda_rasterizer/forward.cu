		...
		float *__restrict__ out_weight_sum)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && render && j < min(BLOCK_SIZE, toDo); j++)
		{
            ...
            //printf("gaussian_id = %d scales[gaussian_id] = (%f,%f,%f) \n", gaussian_id, scales[gaussian_id][0], scales[gaussian_id][1], scales[gaussian_id][2]);
            ...
            // save the previous and post gaussian of the anchor gaussian
            //float scale_max = scales[gaussian_id][0]
            ...
            //printf("scales[gaussian_id]", scales[gaussian_id]);
...
void FORWARD::render(
	...
	float *out_T)
{
    printf("I am printing in forward.cu");
