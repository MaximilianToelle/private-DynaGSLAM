...
class Tracker(object):
    ...
    def map_preprocess(self, frame, frame_id):
        ...
        invalid_confidence_mask = torch.zeros_like(invalid_confidence_mask).bool()
    ...
    def refine_icp_pose(self, dyna_mask):
        ...
        if tracking_success and self.orb_useicp:
            ...
            valid_orb_mask = (~dyna_mask).astype(np.uint8) * 255
    ...
    def tracking(self, frame, frame_map, semantic, semantic_old):
        ...
        if self.use_gt_pose:
            ...
            if not self.status["initialized"]:
                ...
                if self.use_orb_backend:
                    pose_t1_w = self.refine_icp_pose(semantic)
        ...
        self.rgb_old = self.curr_frame['color_map']
    ...
    def save_traj(self, save_path):
        ...
        # Transform the pose_es to the same starting point and view as the pose_gt, by default pose_es assumes starts from (0,0,0)
        compen_trans = np.linalg.inv(pose_gt[0])
        pose_es = pose_es @ pose_gt[0]
        # Compute the translation vector
        
        
        # Extract xyz trajectories
        xyz_gt = pose_gt[:, :3, 3]  # Ground truth trajectory (convert to numpy)
        xyz_est = pose_es[:, :3, 3] # Estimated trajectory (convert to numpy)
        
        xyz_est = (xyz_est-xyz_gt[0])@np.array([[0, 0, 1],
              [1, 0, 0],
              [0, 1, 0]]) #+ xyz_gt[0]
        #xyz_gt = xyz_gt - xyz_gt[0]
        xyz_est = xyz_est[:, [1, 2, 0]]
        xyz_est += xyz_gt[0]
        xyz_est[0] = xyz_gt[0]

        ates_ba = self.eval_total_ate(xyz_est, xyz_gt)
        print("ate: ", ates_ba[-1])
        
         # Function to create line segments for PyVista
        ...
        # Create line segments for ground truth and estimation
        gt_segments = create_line_segments(xyz_gt)
        est_segments = create_line_segments(xyz_est)

        # Create a PyVista plotter
        plotter = pv.Plotter()

        # Add ground truth trajectory
        plotter.add_lines(gt_segments, color="blue", width=2, label="Ground Truth")
        plotter.add_points(xyz_gt, color="blue", point_size=10)

        # Add estimated trajectory
        plotter.add_lines(est_segments, color="red", width=2, label="Estimation")
        plotter.add_points(xyz_est, color="red", point_size=10)
    ...
    def eval_ate(self, pose_es, pose_gt, frame_id=-1):
        pose_es = np.stack(pose_es, axis=0)[:frame_id]
        pose_gt = np.stack(pose_gt, axis=0)[:frame_id]
